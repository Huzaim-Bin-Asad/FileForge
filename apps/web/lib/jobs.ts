...
/**
 * Creates a job whose durable input is already recorded — used by the
 * asynchronous `/api/v1/convert` flow (Phase 4) instead of `createJob`.
 *
 * The id is generated by the caller (not the database default) so the
 * caller can build the job's Blob path and hand the job to Vercel Queues
 * *before* this insert runs — see app/api/v1/convert/route.ts. Call this
 * only after both of those already succeeded, so that every row this
 * creates is, from the moment it exists, both durably stored in Blob and
 * known to Queues: a "queued" job can never be missing either.
 */
...
/**
 * How long a job may sit "processing" with no update before a claim attempt
 * is allowed to treat it as abandoned (the process that claimed it crashed,
 * hit its execution limit, or was otherwise killed without ever reaching
 * `completeJob`/`failJob`). Generously above the slowest converter's
 * expected running time — see lib/jobProcessor.ts and the converter-by-
 * converter table in the Phase 4 report.
 *
 * This is a backstop for the case where nothing ran the failure path at
 * all. A handler that runs and fails normally calls `requeueJob` instead,
 * which makes the job claimable again immediately.
 */
export const STALE_PROCESSING_MS = 10 * 60 * 1000;

/**
 * Atomically claims a job for processing: moves "queued" (or a "processing"
 * job whose lease has gone stale, see `STALE_PROCESSING_MS`) to
 * "processing" and returns the full row, or returns null if there was
 * nothing this caller was allowed to claim — the job doesn't exist, is
 * already in a terminal state, or is actively owned by another still-live
 * claim. Only one concurrent caller can ever get a non-null result for the
 * same job, because the transition happens in a single guarded UPDATE:
 * Postgres serializes concurrent updates to the same row, so of any two
 * callers racing this statement, only the one whose UPDATE commits first
 * can match the WHERE clause — the second finds the row already
 * "processing" with a fresh `started_at` and claims nothing.
 */
export async function claimJobForProcessing(jobId: string) {
  ...
  const [row] = await db
    ...
    .where(
      and(
        eq(jobs.id, jobId),
        or(eq(jobs.status, "queued"), and(eq(jobs.status, "processing"), lt(jobs.startedAt, staleCutoff)))
      )
    )
    .returning();
  return row ?? null;
...
/**
 * Reverts a claimed job back to "queued" after a retryable failure, so the
 * next delivery can claim it immediately rather than waiting out
 * `STALE_PROCESSING_MS`. Guarded to only apply while the job is still
 * "processing" (i.e. still this caller's claim) — never resurrects a job
 * some other caller has already moved to a terminal state.
 */
